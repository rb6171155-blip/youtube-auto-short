...
## 背景動画選定の検証履歴
1. **自然・癒し系背景動画方式（背景動画001 / Baseline）**:
   - 若葉・木漏れ日・青空・草原・水面・山等の高品質自然映像（NATURE_QUERY_POOLS 全24クエリ）を自動取得し、人物・医療等のNGワードを除外。
   - プリセット「背景動画001 (bg_video_001)」として完全保存済み。本プロジェクトの基準映像（Baseline）として本番稼働中。
2. **Pexels人物動画（日本人/アジア人）テスト結果（2026-09-09）**:
   - 目的：台本連動による日本人・アジア人人物動画の適用可能性を検証。
   - 実施内容：代表3テーマ（外来リハ、通所リハ、理念）でPexels階層検索・ダウンロード・Shorts動画合成を実施。
   - 判定：**不採用（本番導入見送り）**。
   - 理由：実際の完成動画を目視評価した結果、映像品質・構図・安定性が自動運用基準に達しておらず、現行の自然背景方式を上回る品質が確認できなかったため。
   - 方針：現行の自然・癒し系背景動画方式をBaselineとして100%維持。今後の改善検討も自然背景Baselineを基準とする。
